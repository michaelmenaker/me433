...
// drawChar function
void drawChar(unsigned short x0, unsigned short y0, int character, unsigned short color){
    int x, y;
    for(x=0;x < 5; x++){
        char currentByte = ASCII[character-32][x];
        for(y=0;y < 8; y++){
            if(currentByte & (1<<y)){
...
// drawString function
void drawString(unsigned short x0, unsigned short y0, unsigned short color, unsigned char* message){
    int i = 0;
    while(message[i]){
        drawChar(x0+6*i, y0, message[i], color);
        i++;
    }
}